...
// IMPORTANT NOTE: This is an SDK addon provided by AngelScript, not a self written version from Vertex.
...
bool CScriptArray::Equals(const void *a, const void *b, asIScriptContext *ctx, SArrayCache *cache) const
{
	...
	else
	{
		...
		if( cache && cache->eqFunc )
		{
			...
			if( subTypeId & asTYPEID_OBJHANDLE )
			{
				r = ctx->SetObject(*((void**)const_cast<void*>(a))); assert(r >= 0);
				r = ctx->SetArgObject(0, *((void**)const_cast<void*>(b))); assert(r >= 0);
			}
			else
			{
				r = ctx->SetObject(const_cast<void*>(a)); assert(r >= 0);
				r = ctx->SetArgObject(0, const_cast<void*>(b)); assert(r >= 0);
			}
