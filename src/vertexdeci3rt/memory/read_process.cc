...
extern "C" VERTEX_EXPORT StatusCode VERTEX_API vertex_memory_read_process(const std::uint64_t address, const std::uint64_t size, char* buffer)
{
    ...
    const SNRESULT result = SNPS3ProcessGetMemory(ctx->module.targetNumber, ppuUnit, ctx->module.processId, threadId, address, size, reinterpret_cast<BYTE*>(buffer));
    if (SN_SUCCEEDED(result))
    {
        return StatusCode::STATUS_OK;
    }

    g_pluginRuntime->vertex_log_error("Failed to read memory. SNRESULT: %d", result);
