...
namespace
{
    ...
    {
        const HANDLE process = get_native_handle(); // NOLINT
        if (!process || process == INVALID_HANDLE_VALUE)
        {
            return ProcessArchitecture::UNKNOWN;
        }

        auto* IsWow64Process2Func = reinterpret_cast<BOOL (WINAPI*)(HANDLE, USHORT*, USHORT*)>(
            GetProcAddress(GetModuleHandle(TEXT("kernel32.dll")), "IsWow64Process2"));
