...
namespace EventInternal
{
    ...
    PluginRuntime::DisasmMode get_disasm_mode(const PluginRuntime::ProcessArchitecture arch)
    {
        switch (arch)
        {
    ...
    StatusCode register_architecture_metadata(const PluginRuntime::ProcessArchitecture arch)
    {
        ...
        const StatusCode status = PluginRuntime::register_architecture(g_pluginRuntime, arch);
        if (status == STATUS_OK)
        {
